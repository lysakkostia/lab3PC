...
struct Metrics {
    ...
    void record_execution_time( double seconds )
    {
        double current = total_execution_time.load();
        while ( !total_execution_time.compare_exchange_weak( current, current + seconds ) );
    }
};